...
void
EventHandler::keydown(int keysym) {
    keymap[keysym] = true;
}

void
EventHandler::keyup(int keysym) {
    keymap[keysym] = false;
}

bool
EventHandler::isDown(int keysym) {
...
void
EventHandler::process_key_press() {
    int net_size;
    switch (e.key.keysym.sym) {
    // Open custom picture
    case SDLK_o:
        // Save current level's network size to restore it in new level
...
LevelLoader::next() {
    // If file is open and there are more level lines, load fromn there.
    ...
    // Else promt with file dialog
    return nextFromDialog();
}

// see https://github.com/mlabbe/nativefiledialog
std::unique_ptr<Puzzle>
...
Game::run() {
    ...
    while (!done) {
        ...
        // If player presses space, show original picture
        if(eventHandler.isDown(SDLK_SPACE)) {
            currentLevel->texture.render();
        } else {
            currentLevel->render();

            // Render red border around selected tile
            ...
            // If puzzle is solved, show original picture for two seconds
            // to let user celebrate their success before the next level
